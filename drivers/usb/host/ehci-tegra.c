...
static void fdt_decode_usb_phy(struct udevice *dev)
{
	...
	utmip_config->hssync_start_delay = ofnode_read_u8_default(
		usb_phy_node, "nvidia,hssync-start-delay", 0x9);

	utmip_config->elastic_limit = ofnode_read_u8_default(
		usb_phy_node, "nvidia,elastic-limit", 0x10);

	utmip_config->idle_wait_delay = ofnode_read_u8_default(
		usb_phy_node, "nvidia,idle-wait-delay", 0x11);

	utmip_config->term_range_adj = ofnode_read_u8_default(
		usb_phy_node, "nvidia,term-range-adj", 0x7);

	utmip_config->xcvr_lsfslew = ofnode_read_u8_default(
		usb_phy_node, "nvidia,xcvr-lsfslew", 0x0);

	utmip_config->xcvr_lsrslew = ofnode_read_u8_default(
		usb_phy_node, "nvidia,xcvr-lsrslew", 0x3);

	utmip_config->xcvr_hsslew = ofnode_read_u8_default(
		usb_phy_node, "nvidia,xcvr-hsslew", 0x8);

	utmip_config->hssquelch_level = ofnode_read_u8_default(
		usb_phy_node, "nvidia,hssquelch-level", 0x2);

	utmip_config->hsdiscon_level = ofnode_read_u8_default(
		usb_phy_node, "nvidia,hsdiscon-level", 0x1);

	if (!utmip_config->xcvr_setup_use_fuses) {
		ofnode_read_u8(usb_phy_node, "nvidia,xcvr-setup",
